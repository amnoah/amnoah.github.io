# Genesee County Healthcare Project

The goal of this project is to make the information researched about Genesee County's medical past more accessible via
different interactive designs. These systems are all used best alongside the content that we write.

This is still very early in development and is missing a lot of data.

# Licensing

You may view this project's license [here](./LICENSE.md).

In brief, this project is dual-licensed under the GNU Affero General Public License v3.0 (AGPLv3) and the PolyForm Noncommercial License 1.0.0.

Commercial users may use this project under the AGPLv3 license.

Non-commercial users may choose to use this project under either the AGPLv3 license or the PolyForm Noncommercial License.'